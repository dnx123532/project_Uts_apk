...
class FoodProvider extends ChangeNotifier {
  ...
  Future<void> _refreshFromNetwork() async {
    try {
      ...
      try {
        final items = await _firestore.getFoodItems().timeout(const Duration(seconds: 8));
        if (items.isNotEmpty) {
          _foodItems = items;
          notifyListeners();
          _db.cacheFoodItems(items).catchError((_) {}); // Update SQLite
          return;
        }
      } catch (_) {}
      ...
      try {
        ...
        if (items.isNotEmpty) {
          _foodItems = items;
          notifyListeners();
          _db.cacheFoodItems(items).catchError((_) {});
        }
      } catch (_) {}
    } catch (_) {}
  }
